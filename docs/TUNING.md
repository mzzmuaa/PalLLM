...
| `BaseUrl` | Root URL of the inference endpoint. Use `/v1/` for most OpenAI-compatible servers, including TensorRT-LLM, and `/v3/` for OpenVINO Model Server. | `http://127.0.0.1:8080/v1/` | — | — | The configured `models` probe fails and Foundry Local `/openai/models` fallback has nothing. | Same. | Confirm `curl <BaseUrl>models` returns JSON; for TensorRT-LLM also confirm `/health`, for OpenVINO this is `/v3/models`, and for Foundry Local also confirm `curl <serviceRoot>/openai/models`. | Operator | No |
