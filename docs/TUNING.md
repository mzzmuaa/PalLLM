...
Controls the live model-call path. **Off by default.** Point it at the bundled local llama.cpp `llama-server` (`pal connect llamacpp`, the only local engine) or, on below-reference hardware, an OpenAI-compatible cloud API (`pal connect cloud`), then flip `Enabled` to route chat through it; if inference fails or is bypassed, the fallback director takes over.
...
metadata. Provider labels stay low-cardinality: an `openai.com` host resolves
to `openai`, the bundled llama.cpp engine (an `llamacpp`/`llama-cpp` host or a
loopback/LAN endpoint on the default port 8080) resolves to `llama.cpp`, and
every other OpenAI-compatible endpoint — including the cloud escape path —
remains `openai_compatible`. The sidecar host also exports bounded recent-window
...
| `BaseUrl` | Root URL of the inference endpoint. Use `/v1/` — the bundled llama-server and OpenAI-compatible cloud APIs both expose it. | `http://127.0.0.1:8080/v1/` | — | — | The `/v1/models` probe fails and tier discovery has nothing. | Same. | Confirm `curl <BaseUrl>models` returns JSON. | Operator | No |
| `Model` | Static fallback when `ModelTiers` is empty. | `Qwen3.6-35B-A3B-UD-Q8_K_XL` | — | — | N/A | Load the model id on your endpoint before flipping `Enabled`. | `pal_active_model_tier` MCP tool or `/api/health`. | Operator | No |
| `ApiKey` | Bearer token for hosted OpenAI-compat endpoints. | `null` | — | — | N/A | N/A | Endpoint-specific. | Operator | No |
| `PrefixCacheSalt` | Optional OpenAI-compatible `cache_salt` forwarded on chat-completions requests. Use one stable, non-secret value per player/save/profile trust domain when a shared endpoint needs prefix-cache isolation. | `null` | `null` | 128 chars | Shared endpoint can reuse prefixes across trust domains if the server allows it. | One-salt-per-request fragmentation destroys prefix-cache hit rate. | Send two same-prefix requests with matching and different salts; confirm reuse only within the matching salt. | Operator | No |
...
| `TopK` | Optional local-runtime `top_k` sampler hint. Omitted unless configured so strict endpoints stay portable. | `null` (code); shipped `appsettings.json` sets `20` for the canonical Qwen3.6 profile | `1` | `65536` | No top-k cap beyond endpoint defaults. | Too small can make replies flat; unsupported endpoints can reject it. | Replay the same companion/task routes with and without the field; compare style, loop rate, token count, p95 latency, and fallback counters. | Designer | Yes |
| `MinP` | Optional local-runtime `min_p` sampler hint. Omitted unless configured so strict endpoints stay portable. | `null` (code); shipped `appsettings.json` sets `0.0` for the canonical Qwen3.6 profile | `0` | `1` | No minimum relative probability filter beyond endpoint defaults. | Too high can prune useful low-probability tactical wording. | Replay creative and strict routes; verify accepted request shape, useful style delta, stable parse success, p95 latency, and fallback counters. | Designer | Yes |
...
| `ThinkingTokenBudget` | Optional OpenAI-compatible `thinking_token_budget` cap for endpoints launched with a reasoning parser. The field is omitted unless configured, and startup rejects zero or negative values. | `null` | `1` | `int.MaxValue` | Reasoning models can spend most of `MaxTokens` on hidden reasoning before producing visible text. | Unsupported endpoints can reject the field; too-low budgets can truncate useful reasoning or return weak final answers. | Replay the same reasoning route with no budget and with the configured budget; record reasoning-parser config, accepted request shape, visible/reasoning token usage, p95 latency, and fallback counters. | Operator | No |
...
| `RequestPriority` | Optional OpenAI-compatible `priority` request hint for foreground scheduling. The field is omitted unless configured; lower values are more urgent on endpoints launched with priority scheduling. | `null` | `int.MinValue` | `int.MaxValue` | Shared servers run first-come-first-served unless their own scheduler settings help short prompts. | Non-zero values can be rejected by FCFS-only servers or strict endpoints that reject the field; overusing foreground values can starve proof/docs lanes. | Queue one short companion turn beside one long proof/docs prompt; confirm lower priority value wins queue time and fallback counters stay stable. | Operator | No |
...
| Prompt-level `StructuredOutputs` | Internal `InferencePrompt` hook for endpoint-specific `structured_outputs` canaries (the bundled llama-server exposes these). Use it for route-owned choice, regex, JSON, grammar, or structural-tag constraints only after the exact endpoint proves support. | `null` | `null` | valid JSON | Engine-specific constrained-decoding shapes stay unavailable outside `response_format`. | Strict endpoints that don't implement the extension can reject the field; stale or over-broad constraints can hide parser failures. | Replay the exact route with and without `structured_outputs`; record accepted request shape, backend id, parse/schema validation, p95 latency, token usage, and fallback counters. | Programmer | Yes |
...
| `WarmupIntervalSeconds` | Optional periodic keepalive cadence. `0` disables periodic keepalives and warms only on startup + tier changes. Recent successful live chat on the same model suppresses the keepalive tick inside this window. (llama-server normally keeps the loaded model resident for the server's lifetime; tune idle unload with `--sleep-idle-seconds` server-side instead.) | `0` | `0` | `86400` | Local inference runtime unloads the active model during idle gaps. | Extra background inference traffic for a lane that may not need to stay hot. | Set to `300`, chat once, then confirm `InferenceWarmup.LastLiveInferenceAtUtc` is populated and the next `periodic_keepalive` skip is visible in `StatusMessage`. | Operator | No |
...
`pal connect llamacpp -WriteConfig` writes the local `/v1/` BaseUrl and the
canonical per-family sampler profile into `appsettings.json`; `pal connect
cloud` does the same for an OpenAI-compatible cloud API on below-reference
hardware.

`llama-server` keeps the loaded model resident for the lifetime of the server
process; idle behaviour is controlled by server flags such as
`--sleep-idle-seconds`, not by any PalLLM per-request setting. Use PalLLM
...
| `Model` | Exact model tag the inference endpoint exposes. | — | The `id` the endpoint surfaces on `/v1/models` (for the bundled llama-server, the GGUF model name). The operator's curated identifiers are unsloth UD-* strings (e.g. `Qwen3.6-35B-A3B-UD-Q8_K_XL`) — see `LOCAL_MODELS_INVENTORY.md`. |
...
| `BaseUrl` | Multimodal endpoint root. Often same host as Inference. | `http://127.0.0.1:8080/v1/` | — | — | — | — | `curl <BaseUrl>models`. | Operator | No |
| `Model` | Vision model id. | `Qwen3.6-35B-A3B-UD-Q8_K_XL` | — | — | — | Larger models = more latency per chat turn. | — | Operator | No |
...
| `UseStructuredOutputs` | Send `response_format: json_schema` to endpoints that support it (the bundled llama-server does). | `true` | — | — | Model returns prose → extraction brittle. | Endpoint may reject unknown field → fall back to prose. | — | Designer | No |
