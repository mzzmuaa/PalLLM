...
public static class PalLlmFeatureCatalog
{
    ...
    [
        ...
        new FeatureDescriptor
        {
            ...
            Notes = "Endpoints: POST /api/vision/describe (freeform), POST /api/vision/world-state (structured JSON that optionally merges into the live snapshot). ChatRequest.ImageBase64 splices a one-sentence visual summary into the system prompt. Outgoing image_url parts can carry stable palllm-image-sha256-* media-cache ids for repeated screenshot lanes, with PalLLM:Vision:UseMediaCacheIds=false as the strict-endpoint opt-out. PalLLM:Vision:MultimodalProcessor can emit mm_processor_kwargs (min/max pixels, max_soft_tokens, fps) only after endpoint proof. Off by default; configure PalLLM:Vision to enable.",
        },
        new FeatureDescriptor
        {
            ...
            Source = "PalLLM runtime",
            Status = "ready",
            Summary = "World-state extraction forwards an OpenAI-style response_format json_schema so OpenAI-compatible endpoints that support structured outputs (including llama.cpp) constrain the model to the PalLLM world-state schema instead of returning prose.",
        ...
        new FeatureDescriptor
        {
            ...
            Notes = "Configured via `PalLLM:Inference:ModelTiers[]` in appsettings.json with `{Id, Model, Priority, Description?}`. On startup the orchestrator seeds the active tier with the lowest-priority entry in list order so the very first chat request works before any probe completes - the player sees replies from second one of the session. A `ModelTierUpgradeWorker` IHostedService re-probes every `TierProbeIntervalSeconds` (default 30s) and atomically swaps the active tier when a higher-priority one appears, emitting a `pal.model_tier.transition` OpenTelemetry span so operators can see the graduation in their tracing backend. Probe queries the OpenAI-compatible `/v1/models` endpoint, which llama.cpp/llama-server (PalLLM's only local engine) and the OpenAI-compatible cloud escape path both expose. Pass 436 removed the Foundry Local `/openai/models` candidate and Pass 346 removed the Ollama-native `/api/tags` fallback - `/v1/models` is now the sole probe. Transient probe failures keep the current tier (no thrashing). Backwards compatible - empty `ModelTiers` list disables orchestration entirely and `Inference.Model` is used verbatim.",
        },
        new FeatureDescriptor
        {
            ...
            Notes = "Exposed on `GET /api/inference/collaboration`, `POST /api/inference/collaboration/plan`, `pal models serving`, the `pal_model_collaboration` and `pal_plan_model_collaboration_task` MCP tools, and the `palllm://model/collaboration` MCP resource. The planner classifies configured models by operating style and emits a deterministic capability profile for each lane (modalities, structured-output/tool-call/speculative-decoding fit, prefill/cache/speculation hints). Serving guidance is llama.cpp-only: `pal connect llamacpp` setup for GGUF lanes, llama-server prompt-cache (cache_prompt, --cache-reuse, -cram, slot count), quantized-KV (-ctk/-ctv), --sleep-idle-seconds idle behaviour, --mmproj multimodal, n-gram and model-native (Qwen3.6/Gemma 4) --spec-type speculation with strict-route no-spec proof, and the OpenAI-compatible cloud escape path (`pal connect cloud`) for below-reference hardware. It also covers the optional OpenAI-compatible request passthroughs PalLLM forwards only when configured (response_format / structured_outputs, tools / tool_choice, prediction, logprobs, modalities / audio, prompt-cache key + retention, store, metadata, request-id header, sampler bounds, token-budget field, stop sequences, multimodal mm_processor_kwargs + media-cache UUIDs), llama.cpp Qwen Omni talker/code2wav /v1/audio/speech proof, Gemma 3n / Gemma 4 audio-in token budgets, primary-source/runtime capability-handshake and model-artifact provenance receipts, local hash-pinned LoRA/personality-adapter guardrails, serving optimizations, runtime guards, media SSRF controls, and promotion verification checks. It tailors sequential-versus-parallel lane use to the available hardware budget and emits authority boundaries, risk-based routing policies, concrete task-routing decisions, and a qualification suite for shadow-testing fresh quants before promotion. The intended posture is a fast worker lane backed by a slower dense planner and final judge, all served by llama.cpp; the scope is intentionally narrow: Palworld-mod runtime work, not a generic media or product studio.",
        },
        new FeatureDescriptor
        {
